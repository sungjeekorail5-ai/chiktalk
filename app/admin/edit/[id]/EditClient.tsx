...
export default function EditClient({
  ...
  const handleScreenshotsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      setNewScreenshots((prev) => [...prev, ...arr]);
  ...
  const handleUpdate = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const uploadedNewScreenshots: string[] = [];
      if (newScreenshots.length > 0) {
        for (let i = 0; i < newScreenshots.length; i++) {
          setUploadProgress(
            `스크린샷 업로드 중... (${i + 1}/${newScreenshots.length})`
          );
      ...
      updateData.screenshotUrls = [
        ...existingScreenshots,
        ...uploadedNewScreenshots,
      ];
        ...
        <div className="md:bg-white md:p-10 md:rounded-3xl md:border md:border-gray-100">
          ...
          >
            ...
            <div className="space-y-2">
              ...
              {(existingScreenshots.length > 0 ||
                ...
                <div className="flex gap-2 overflow-x-auto py-1 -mx-5 px-5 md:mx-0 md:px-0">
                  ...
                  {newScreenshotPreviews.map((src, index) => (
                    <div
                      key={`new-${index}`}
                      className="relative flex-shrink-0 w-20 h-32 rounded-xl overflow-hidden border-2 border-blue-500"
                    >
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        src={src}
                      ...
                      <span className="absolute bottom-1 left-1 bg-blue-500 text-white text-[8px] px-1 rounded font-bold">
                        NEW
                      </span>
                    </div>
                  ))}
              ...
              <label className="flex items-center justify-center w-full h-12 bg-gray-50 rounded-xl cursor-pointer">
            ...
            <div className="space-y-2">
              ...
              <div className="px-4 py-3 bg-gray-50 rounded-xl text-sm font-medium">
                {newFileName ? (
